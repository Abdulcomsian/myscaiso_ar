                ...
                <div class="row">
                    <div class="col-lg-4">
                        ...
                        <div class="nav nav-pills faq-nav" id="faq-tabs" role="tablist" aria-orientation="vertical">
                            @foreach ($all_cates as $cate)                            
                                <a href="#tab{{ $cate->id }}" class="nav-link" style="text-align: right;" data-toggle="pill" role="tab"
                                    aria-controls="tab{{ $cate->id }}" aria-selected="true">
                    ...
                    <div class="col-lg-8">
                        <div class="tab-content" id="faq-tab-content">
                            <?php $x = 1; ?>
                            @foreach ($all_cates as $cate)
                                <div class="tab-pane {{ $x == 1 ? 'show active' : '' }}" id="tab{{ $cate->id }}"
                                    role="tabpanel" aria-labelledby="tab{{ $cate->id }}">
                                    <div class="accordion" id="accordion-tab-{{ $cate->id }}">
                                        @foreach ($all_faqs as $faq)
                                            @if ($faq->category == $cate->id)
                                                <div class="card category-{{ str_replace(' ', '-', $cate->name) }}">
                                                    <div class="card-header"
                                                        id="accordion-tab-{{ $cate->id }}-heading-{{ $faq->id }}">
                                                        <h5>
                                                            <p>
                                                            <button class="btn btn-link" style="text-align:left;"
                                                                ...
                                                                aria-expanded="false"
                                                                aria-controls="accordion-tab-{{ $cate->id }}-content-{{ $faq->id }}">{{ $faq->question }}</button>
                                                            </p>
                                                        </h5>
                                                    </div>
                                                    <div class="collapse"
