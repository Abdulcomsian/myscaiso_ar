		...
		<?php if(Request::is('products*')): ?>
			...
			<script src="<?php echo e(asset('/assets/app/custom/product/listing.js')); ?>" type="text/javascript"></script>
		<?php endif; ?>

		<?php if(Request::is('user*') || Request::is('customer*') || Request::is('agent*')): ?>
			
			<script type="text/javascript" src="<?php echo e(asset('/js/jquery.uploadPreview.min.js')); ?>"></script>
			<!--begin::Product Page Scripts(used by this page) -->
			<script src="<?php echo e(asset('/assets/app/custom/user/listing.js')); ?>" type="text/javascript"></script>

			<!--begin::Page Scripts(used by this page) -->
			<!-- <script src="<?php echo e(asset('/assets/dropzone/dist/dropzone.js')); ?>" type="text/javascript"></script> -->

			<!--end::Page Scripts -->
		<?php endif; ?>

		<!--begin::Page Scripts(used by this page) -->
		<script src="<?php echo e(asset('/assets/app/custom/general/crud/forms/widgets/bootstrap-datepicker.js')); ?>" type="text/javascript"></script>
		...
		<script type="text/javascript">
			...
			jQuery('.addPlane').click(function(){
				...
				var cell1 = row.insertCell(0);
		      	...
		      	cell1.innerHTML='<span>1</span>';
		      	cell2.innerHTML='<div class="plane_input"><input type="text" id="amount" name="amount" class="form-control" placeholder="Enter Your Total Amount"></div>';
		      	cell3.innerHTML='<div class="plane_input"><input type="text" id="payable_amount" name="payable_amount" class="form-control" placeholder="Enter Your Payable Amount"></div>';
		      	cell4.innerHTML='<div class="input-group date"><input type="text" name="dob" class="form-control" readonly value="<?php echo e(\Carbon\Carbon::now()->format('d/m/Y')); ?>" id="dob" /><div class="input-group-append"><span class="input-group-text"><i class="la la-calendar"></i></span></div></div>';
		      	cell5.innerHTML='<span class="kt-badge  kt-badge--warning kt-badge--inline kt-badge--pill">Pending</span>';
			})
		</script>
		<script type="text/javascript">
			jQuery('.addNote').click(function(){
				var table = document.getElementById("kt_customer_note_body");
				var row = table.insertRow(0);
				var cell1 = row.insertCell(0);
		      	var cell2 = row.insertCell(1);
		      	var cell3 = row.insertCell(2);
		      	...
		      	cell3.innerHTML='<div class="input-group date"><input type="text" name="dob" class="form-control" readonly value="<?php echo e(\Carbon\Carbon::now()->format('d/m/Y')); ?>" id="dob" /><div class="input-group-append"><span class="input-group-text"><i class="la la-calendar"></i></span></div></div>';
			})
			$(function(){
		...
		<script type="text/javascript">
			...
			function requirementFrom(){
				if($(".requirments_from_div").css("display")==="block"){
					...
					$(".requirments_from_div").css("display","block");
				}
			}
			function processAuditForm(){
				...
				if($(".process_audit_from_div").css("display")==="block"){
					...
					$(".process_audit_from_div").css("display","block");
				}
			}
			function qmsAudit(){
				resetForm();
				if($(".qms_audit_from_div").css("display")==="block"){
					...
					$(".qms_audit_from_div").css("display","block");
				}
			}
			function nonConformities(){
				if($(".non_conformities_from_div").css("display")==="block"){
					$(".non_conformities_from_div").css("display","none");
				}
				else{
					$(".non_conformities_from_div").css("display","block");
				}
			}
			function customerForm(){
				if($(".customer_from_div").css("display")==="block"){
					$(".customer_from_div").css("display","none");
				}
				else{
					$(".customer_from_div").css("display","block");
				}
			}
			function customerReview(){
				if($(".customer_review_from_div").css("display")==="block"){
					$(".customer_review_from_div").css("display","none");
				}
				else{
			...
			function accidentRiskForm(){
				...
				if($(".accident_risk_from_div").css("display")==="block"){
					...
					$(".accident_risk_from_div").css("display","block");
				}
			}
			function riskAssessment(){
				...
				if($(".risk_assessment_from_div").css("display")==="block"){
					...
					$(".risk_assessment_from_div").css("display","block");
				}
			}
			function workInstructionFrom(){
				if($(".work_instruction_from_div").css("display")==="block"){
					$(".work_instruction_from_div").css("display","none");
				}
				else{
		...
		<?php echo $__env->yieldContent('myscript'); ?><?php /**PATH C:\xampp\htdocs\myscaiso_ar\resources\views/admin/dashboard/includes/foot.blade.php ENDPATH**/ ?>